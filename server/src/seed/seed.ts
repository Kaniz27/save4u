...
// heroImage values are Unsplash source placeholders — swap for real Save4u photography.
const services = [
  {
  ...
  },
  {
    ...
    highlights: [
      ...
      },
      {
        ...
        description:
          "Frequent thermostat adjustments cost more than a steady, comfortable temperature. A consistent setting saves more over a year than constant tweaking.",
      },
    ],
  ...
  },
  {
    ...
    subServices: [
      ...
      },
      {
        ...
        detail:
          "Get ahead of demand — buy in bulk, replace ageing equipment, or add a new product line — without waiting on cash flow to catch up first.",
      },
      {
        icon: "TrendingUp",
    ...
    highlights: [
      {
        ...
        description:
          "One clear cost agreed before you accept — no APR calculations and no surprise charges added later.",
      },
      {
        icon: "HeartHandshake",
        title: "No Restrictions on Spend",
        description:
          "Use the funds however your business needs them, with no hidden fees for how or where you spend it.",
      },
      {
        icon: "TrendingUp",
        title: "Support at Every Stage",
        description:
          "From a first-time advance to repeat funding as you grow, we structure each advance around where your business is right now.",
      },
    ],
    ...
    highlights: [
      {
        ...
        description:
          "Compared to traditional advertising, digital marketing typically costs less and is easier to track against actual return.",
      },
      {
        icon: "BarChart3",
        title: "Results You Can Measure",
        description:
          "Real-time tracking means you always know what's working, so budget goes where it performs — not where it feels like it should.",
      },
      {
        icon: "Users",
        title: "Reach the Right Audience",
        description:
          "Target by location, interest, and behaviour, so your budget reaches people actually likely to buy, not just anyone scrolling past.",
      },
    ],
...
const reviews = [
  { clientName: "James Carter", companyName: "Carter & Co Retail", rating: 5, quote: "Switching our card machine through Save4u cut our processing fees noticeably within the first month.", serviceUsed: "payment-solution", isPublished: true },
...
async function seed() {
  ...
  if (existingReviews === 0) {
    ...
    console.log(`[seed] inserted ${reviews.length} sample reviews`);
  } else {
    console.log("[seed] reviews already exist, skipping");
  }

  await SiteSettings.findOneAndUpdate({}, {}, { upsert: true });
  ...
  if (!existingAdmin) {
    ...
    await Admin.create({ email: env.seedAdminEmail, passwordHash, role: "admin" });
    console.log(`[seed] created admin account: ${env.seedAdminEmail}`);
    console.log(`[seed] password: ${env.seedAdminPassword} (set SEED_ADMIN_PASSWORD in .env to change)`);
  } else {
